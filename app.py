...
from email_sender import send_credentials_email, send_individual_credentials_email
...
def add_access_grant():
    ...
    indiv_username = (data.get('individual_username') or '').strip()
    indiv_password = (data.get('individual_password') or '').strip()
    indiv_app_url  = (data.get('individual_app_url')  or '').strip()

    # For Individual ID: either all three credential fields are filled, or all blank.
    if access_type == 'Individual ID':
        filled = sum(1 for v in (indiv_username, indiv_password, indiv_app_url) if v)
        if 0 < filled < 3:
            return jsonify({"error": "For Individual ID: fill all three credential fields (username, password, URL), or leave all three blank."}), 400
    ...
    conn.run("""INSERT INTO access_grants (user_name, user_email, user_designation, user_department, app_name, access_type, granted_by, notes)
             ...
             a=data['app_name'], at=access_type,
    ...
    # If Individual ID + all three credential fields provided + admin explicitly
    # opted in via the "Send credentials email" checkbox + we have a target email,
    # log the audit event and fire off the credentials email.
    target_email = (data.get('user_email') or '').strip()
    send_email_flag = bool(data.get('send_email'))
    will_email = (
        access_type == 'Individual ID'
        and indiv_username and indiv_password and indiv_app_url
        and bool(target_email)
        and send_email_flag
    )
    if will_email:
        conn.run("""INSERT INTO audit_logs (user_email, app_name, action)
                    VALUES (:e, :a, 'individual_credentials_emailed')""",
                 e=target_email, a=data['app_name'])
    ...
    return jsonify({
        "success": True,
        "auto_resolved_requests": auto_resolved,
        "credentials_emailed": bool(will_email),
    })
